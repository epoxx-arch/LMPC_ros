...
public:
    ...
    Track(string file_name, nav_msgs::OccupancyGrid& map, bool sparse=false) : space(0.1), map(map){
        space = 0.1;
    ...
    void initialize_width(){
        ...
        for (int i=0; i<centerline_points.size(); i++){
            ...
            // cout<<"----x----="<<centerline_points[i].x<<" "<<centerline_points[i].y<<" "<<centerline_points[i].left_half_width<<" "<<centerline_points[i].right_half_width<<endl;
            // cout<<"----p_left----="<<p_left(0)<<" "<<p_left(1)<<" "<<p_right(0)<<" "<<p_right(1)<<endl;
